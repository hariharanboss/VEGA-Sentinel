...
% Give LaTeX extra stretch to avoid overfull lines
\emergencystretch=1.5em

% ---------- image helper: compiles with or without the image ----------
% Tries PDF first (pdfTeX embeds PDFs natively - avoids libpng issues),
% then PNG, and otherwise shows a labelled placeholder box.
    ...
    \IfFileExists{#1.pdf}{%
      ...
      \IfFileExists{#1.png}{%
        ...
        \fbox{\parbox[c][4.5cm][c]{#2\textwidth}{\centering
          ...
          Upload \texttt{\detokenize{#1}.pdf} or \texttt{\detokenize{#1}.png}\\
          to this Overleaf project, or replace this box\\
          with {\ttfamily\char92 includegraphics}.}}%
      }%
    }%
  \end{center}
}
...
\reportimage{photo_components}{0.75}
...
\reportimage{schematic_circuit}{0.8}
...
\begin{tikzpicture}[
  ...
  \draw[arrow] (valid) -| node[lbl, above, pos=0.25]{No} (discard.west);
  ...
  \draw[arrow] (infer) -- (th);
  \draw[arrow] (th) -| node[lbl, above, pos=0.2]{No} (norm.west);
  \draw[arrow] (norm) |- (conf);
  \draw[arrow] (th) -- node[lbl, right=1mm]{Yes} (conf);
  \draw[arrow] (conf) -- (buz);
\end{tikzpicture}
\caption{Firmware flowchart (one monitoring cycle; the cycle repeats
from \emph{Read sensors}).}
\end{figure}
...
\begin{itemize}[nosep]
  \item \textbf{Firmware:} no external libraries. The VEGA Arduino
    ...
    arithmetic.
  \item \textbf{PC pipeline (training/data tools):} \texttt{pandas},
    \texttt{numpy}, \texttt{scikit-learn}, \texttt{matplotlib},
    \texttt{pyserial}.
\end{itemize}
...
% ============================================================
\begin{description}
  \item[Initialization:] serial at 115200 baud; GPIO0 and GPIO1
    configured; GPIO2 set as output, LOW (buzzer off); A0 analog
    input.
...
\reportimage{vega_sentinel_mq2_comparison}{0.85}
\captionof{figure}{MQ-2 sensor response comparison across sessions.}
\reportimage{vega_sentinel_flame_incident}{0.85}
...
\reportimage{video_qr_screenshot}{0.45}
